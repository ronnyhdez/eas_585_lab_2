...
package_list = c("RStoolbox", "raster", "tidyverse", "sf", "rpart",
                 "rpart.plot", "rasterVis", "mapedit", "mapview", "caret",
                 "forcats", "patchwork", "cluster", "randomForest")
#install.packages(c("RStoolbox", "raster", "tidyverse", "sf", "rpart",
#                   "rpart.plot", "rasterVis", "mapedit", "mapview", "caret",
#                   "forcats", "patchwork", "cluster", "randomForest"))
...
#Set your directory to the lab folder, that way outputs and inputs are saved
#or read from the proper location
#setwd("~/Lab_2_image_classification_R/")
setwd("E:/eas451/Lab_2_image_classification_R/")
...
nlayers(image) #this will return the number of layers

#How many layers should we have?

#Now lets check to make sure our spatial reference is still valid, otherwise our image is lost spatially
crs(image)
...
#What bands are used in the True Color Composite? Refer to the band list at the top
#ex: Band 9 (Cirrus)
#False Color Composite
par(col.axis="white",col.lab="white",tck=0)
...
##############################################################################################################
#Since we have create the image, we may wish to remove some of the extra files for the sake of memory space
rm(band1)
rm(band2)
rm(band3)
rm(band4)
...
#we can use a general 'garbage collection' to free up some space as well that may be occupied
gc()
##############################################################################################################
...
#What bands are used in the NDVI calculation? Why these bands? Recall the Vegetation Spectrum
...
#We are now going to attempt a Supervised Classification to try and classify each pixel of the image into various classes
#As this is a Supervised Classification we will first need to create a training dataset


#The below line will open up RGB image in the plotting window as an interactive image. You can pan around with the hand and zoom in 
#with the scroll wheel. Get familiar with the controls and movement before moving on.
#Once comfortable, select the option from the left menu for drawing a 'polygon', this will allow you to click and define a polygon
#You are clicking to place the vertices, so be aware that it will be drawing straight lines between points. Once you have defined a polygon
#click on the first point to close it, then you can click 'cancel' to clear the tool and move to the next area 
#Please only do one class at a time, meaning that if you are defining agriculture, just draw all agriculture polygons.
#Once you have draw several polygons for one class, select 'done' in the bottom right

#We need to run an individual instance of the below line for each class we want. Please try to create the following classes:
...
#Save each iteration, be aware that the "id = x", x will represent the order or which instance you are on. For example, if I define 'clouds'
#first, then the id will be 1, id = 1. So be careful of the order and the id number you are using.

# save as agriculture after first iteration
agriculture <- points$finished$geometry %>% st_sf() %>% mutate(class = "agriculture", id = 1)
...
water <- points$finished$geometry %>% st_sf() %>% mutate(class = "water", id = 3)
# save as other vegetation for the fourth
veg <- points$finished$geometry %>% st_sf() %>% mutate(class = "vegetation", id = 4)
...
training_points <- rbind(agriculture, urban, water, veg)


####################################################################################
#You can write these points as a shapefile with: 
write_sf(training_points, "calgary_trainingPoints2.shp", driver = "ESRI shapefile")
#to be read back in later with: 
training_points <- st_read("calgary_trainingPoints2.shp")
####################################################################################
...
B <- ggplot() +
  ...
  scale_color_manual(values = c('lawngreen', 'burlywood', 'darkgreen', 'blue')) +
...
profiles %>% 
  select(-id) %>% 
  gather() %>% 
  mutate(class = rep(c("agriculture", "urban", "water", "vegetation"), 11)) %>% 
...
levels(pr) <- levels(pr)[[1]] %>%
  mutate(legend = c("agriculture","urban","water", "vegetation"))
...
plot(clara_raster, col = topo.colors(4)) #try other colors as well
